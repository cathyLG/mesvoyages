...
class VisiteValidationsTest extends KernelTestCase {
    ...
    /**
     * tester une note invalide (out of range 0-20)
     */
    ...
    /**
     * tester si tempmax peut < tempmin
     */
    public function testValidTempmaxVisite() {
        $this->assertErrors($this->getVisite()->setTempsmin(18)->setTempsmax(20), 0, "min=18, max=20 devrait réussir");
        $this->assertErrors($this->getVisite()->setTempsmin(19)->setTempsmax(20), 0, "min=19, max=20 devrait réussir");
    }

    public function testNonValidTempmaxVisite() {
        $this->assertErrors($this->getVisite()->setTempsmin(20)->setTempsmax(18), 1, "min=20, max=18 devrait échouer");
        $this->assertErrors($this->getVisite()->setTempsmin(20)->setTempsmax(20), 1, "min=20, max=20 devrait échouer");
    }
    ...
    public function assertErrors(Visite $visite, int $expectedErrorNb, string $message = '') {
        self::bootKernel();
        $error = self::$container->get('validator')->validate($visite);
        $this->assertCount($expectedErrorNb, $error, $message);
    }
